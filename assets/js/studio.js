...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvas2D && viewport2D && viewerContainer && canvas3D) {
        ...
        if (btnDownloadSkin) {
            btnDownloadSkin.addEventListener('click', async () => {
                // La descarga local pasa siempre, tenga o no sesión iniciada
                const enlace = document.createElement('a');
                enlace.download = 'skin-craftity.png';
                enlace.href = dataCanvas.toDataURL('image/png');
                enlace.click();

                // Si hay sesión, además ofrecemos guardarla en la Galería del perfil
                const client = window.craftitySupabase;
                if (!client) return;
                const { data: { session } } = await client.auth.getSession();
                if (!session) return; // Sin sesión: no tiene perfil donde guardarla

                const galleryModal = document.getElementById('studioGalleryModal');
                if (galleryModal) galleryModal.classList.add('open');
            });
        }

        // ================= GUARDAR EN LA GALERÍA (modal que aparece tras Descargar Skin) =================
        const galleryModal = document.getElementById('studioGalleryModal');
        const btnGallerySkip = document.getElementById('btnGallerySkip');
        const btnGallerySave = document.getElementById('btnGallerySave');
        const galleryError = document.getElementById('galleryError');

        if (btnGallerySkip && galleryModal) {
            btnGallerySkip.addEventListener('click', () => galleryModal.classList.remove('open'));
        }

        if (btnGallerySave && galleryModal) {
            btnGallerySave.addEventListener('click', async () => {
                const client = window.craftitySupabase;
                const { data: { session } } = await client.auth.getSession();
                if (!session) { galleryModal.classList.remove('open'); return; }

                const titulo = document.getElementById('gallerySkinTitle').value.trim() || 'Sin título';
                const esPublica = document.getElementById('gallerySkinPublic').checked;
                galleryError.textContent = '';
                btnGallerySave.disabled = true;

                dataCanvas.toBlob(async (blob) => {
                    const nombreArchivo = `gallery/${session.user.id}-${Date.now()}.png`;
                    ...
                    if (errorSubida) {
                        galleryError.textContent = 'Error subiendo la imagen: ' + errorSubida.message;
                        btnGallerySave.disabled = false;
                        return;
                    }

                    const { data: urlData } = client.storage.from('skins').getPublicUrl(nombreArchivo);

                    const { error: errorInsert } = await client.from('skins').insert({
                    ...
                    if (errorInsert) {
                        galleryError.textContent = errorInsert.message;
                        return;
                    }
